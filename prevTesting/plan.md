...
4) Переработать систему обновления:  
   ...
   - Основная сложность будет в том, что функции для обновления разных вещей нужно будет как-то связать, иначе придется каждый раз проходить по всему массиву сущностей отдельно для обновления каждой, отдельно для оббновления `entitiesAlive` и т. д.
