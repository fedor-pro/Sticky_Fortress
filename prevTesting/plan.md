...
4) Переработать систему обновления:  
   1. Перенести все обновление в отдельную систему `updateLord`. Что надо перенести:
      ...
      2. Обновление счетчика выделенных и живых ✗ дварфов ✗ и клеток (для дварфов найдено лучшее решение - обновление их статов просто перенесено в entites.c, так как нет смысла проходить по всему массиву (см. ниже) дварфов в main.c для обновления двух переменных)
   ...
   - Основная сложность будет в том, что функции для обновления разных вещей нужно будет как-то связать, иначе придется каждый раз проходить по всему массиву дварфов отдельно для обновления их самих, отдельно для обновления `entitiesAlive` и т. д.
   3. Кроме того, нужно будет что-что придумать насчёт удаления пустой еды и мертвых дварфов из массива, или их перемещения в конец
...
5) Улучшение моментов:
   ...
   3. Улучшить дварфов:
      ...
      ![alt](images/entitiesOwerObstaclesPlan.png)
...
> Общее расширение функционала игры, дварфивизация дварфов,  
