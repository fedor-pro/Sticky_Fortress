...
2) Сделать что-то из этого: ✔ Хранить все переменные вроде `foodOnMap` и `windowSize` в одном большом лорде `datalord` ✗  
 ИЛИ  
                              ...
                              * `worldParamsDataLord` - для  переменных вроде `foodOnMap`, `dwarvesNumber` и т. д. ✔  
                              ...
                              * `logDataLord`         - для  `struct tm *tm, FILE *sourceLogFile`  
                                                      -- решить три проблемы:  
                                                      --- встроить `logDataLord` в `dwarves` и `world` ✔  
...
4) Переработать систему обновления:  
   1. Перенести все обновление в отдельную систему `updateLord`. Что надо перенести: ✔
      ...
      2. ✗ Обновление счетчика выделенных и живых дварфов и клеток ✗ (найдены лучшие решения: статы дварфов перенесены в dwarves.c, статы клеток перенесены в выделения)
   ...
   - Основная сложность будет в том, что функции для обновления разных вещей нужно будет как-то связать, иначе придется каждый раз проходить по всему массиву дварфов отдельно для обновления их самих, отдельно для обновления `dwarvesAlive` и т. д. ✗
   3. Кроме того, нужно будет что-что придумать насчёт удаления пустой еды и ✗ мертвых дварфов ✗ из массива, или их перемещения в конец (в самой Dwarf Fortress мертвые дварфы не удаляются из памяти, в отличии от еды,
   так что этот пункт отменяется)
...
5) Улучшение моментов:
   ...
   3. Улучшить дварфов:
      ...
      ![alt](images/dwarvesOwerObstaclesPlan.png)
